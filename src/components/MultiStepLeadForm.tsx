...
import { z } from "zod";
import { Loader2, ArrowLeft, ArrowRight, CheckCircle2, MessageCircle, Lock } from "lucide-react";
...
/**
 * Formulário multi-step de 2 etapas (removida etapa de "detalhes" — reduz abandono).
 * Etapa 1: Tipo de serviço (clique único)
 * Etapa 2: Nome + WhatsApp (mínimo para contato)
 * Submit via hook useLeadSubmit (lógica centralizada, sem duplicação).
 */
...
const SERVICES_OPTIONS = [
  ...SITE_CONFIG.services.map((s) => ({ label: s.title, value: s.title })),
  { label: "Outro / Não tenho certeza", value: "Outro" },
];
...
export function MultiStepLeadForm({ source = "multistep", onStepChange }: Props) {
  ...
  function next() {
    if (!data.servico) {
      setErrors({ servico: "Escolha uma opção para continuar" });
      return;
    }
      ...
      {step === 2 && (
        <div className="space-y-4 animate-fade-up">
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => { setStep(1); onStepChange?.(1, totalSteps); }}
              disabled={loading}
